/**
 * data.js — All detection data for wearehere privacy assessment.
 *
 * Merges:
 *   - src/data.js (tracker domains, dark patterns, storage patterns, tracking params, fingerprint APIs)
 *   - chrome-extension/network-domains.js (454 tracker domains, 82 broker profiles, domain classification)
 *   - chrome-extension/tos-scanner.js (54 regex patterns across 6 categories)
 */

// =============================================================================
// Page-level detection data (injected into page context as strings)
// =============================================================================

// --- Tracker domains for page-level pixel/iframe detection ---
...
// --- Suspicious localStorage key patterns ---
...
// =============================================================================
// Network-level domain classification (from wearebaked v0.5.1)
// =============================================================================

export const NET_TRACKER_DOMAINS = {
  // Advertising
...
export const NET_DOMAIN_PATTERNS = [
...
// --- Purpose domains (benign) ---
export const NET_PURPOSE_DOMAINS = {
  cdn: [
...
// =============================================================================
// Network domain classification helpers
// =============================================================================
...
export function classifyNetworkDomain(domain) {
  const root = getRootDomain(domain);

  // Pass 1: exact domain DB
  if (NET_TRACKER_DOMAINS[root]) return { category: NET_TRACKER_DOMAINS[root], risky: true };
  if (NET_TRACKER_DOMAINS[domain]) return { category: NET_TRACKER_DOMAINS[domain], risky: true };

  // Check benign purpose domains
...
// --- Common ToS page paths to try ---
export const TOS_PATHS = [
  ...
  '/cookie-policy', '/data-policy',
  '/site/privacy', '/site/terms',
  '/company/privacy', '/company/terms',
  '/en/privacy', '/en/terms',
  '/us/privacy', '/us/legal',
];

// =============================================================================
// ToS scanning (from wearetosed v0.1.0)
// =============================================================================
...
export function scanText(text) {
  const counts = {};
  const seen = {};
  for (const cat in TOS_SCANNERS) counts[cat] = 0;

  const lowerText = text.toLowerCase();

  for (const [category, patterns] of Object.entries(TOS_SCANNERS)) {
    for (const re of patterns) {
      const matches = lowerText.match(new RegExp(re.source, 'gi'));
      if (matches) {
        for (const m of matches) {
          const key = category + ':' + m.slice(0, 40);
          if (!seen[key]) {
            seen[key] = true;
            counts[category]++;
          }
  ...
  const score = Math.min(100, patternCount * 8 + total * 2);
