...
- **Overview leads with protection, not just exposure.** The hero's 4th cell switches from "most exposed" (which duplicated the most-watched line) to **cookies trimmed** this window. A promoted green protection line spans both mechanisms — windowed cookies trimmed + windowed fingerprint surfaces blurred. First-visit events collapse into a single "N new sites" summary so notable changes (new watchers, reach grew) lead the "what changed" feed. The score-over-time chart is replaced with a **browsing exposure** distribution — sites bucketed by risk tier (clean / mixed / hostile) — a truer at-a-glance read than a per-visit zigzag.
