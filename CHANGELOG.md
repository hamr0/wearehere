...
- **Relax offer fired on normal navigation (e.g. typing Google searches).** The rage-reload detector counted *any* same-tab load and keyed on `tab.url`, which lags at the `loading` event — so successive searches looked like repeated reloads of one URL and popped the "relax fingerprint protection" banner. Reworked to the real `chrome.tabs.onUpdated` semantics (verified against captured event streams): a navigation carries `changeInfo.url` and resets the streak; a *url-less* `loading` event is a reload, counted once per load cycle, with a navigation's own sub-events ignored via a timestamp. Fires on the 2nd genuine reload; works even on stuck pages that never fire `complete` (time-based, not `complete`-based). Replaces the earlier "≥2× same URL" heuristic.
- **Terms scan flip-flopped between found and "couldn't find a terms page."** The ToS result cache (`domainTosCache`) was an in-memory object that evaporated on every service-worker / event-page recycle, so a once-found terms page reverted to "couldn't find" the next time the popup opened cold. It's now persisted to `storage.local` (`tosCacheV1`): merge-keeps known privacy/terms axes (never clobbers a found page with a later null), TTL'd (30d found / 1d miss, so misses still re-discover), capped at 500 with oldest-first eviction. The `checkCache` handler is hardened so a throw can't leave the content script's callback hanging (which would skip the scan).
- **Per-site rules over-grouped multi-label public suffixes.** Registrable domains were computed as the last two labels everywhere, so `*.co.uk` and `*.github.io` collapsed into one bucket — relaxing or trusting one site (e.g. `abrahamjuliot.github.io`) silently applied to *every* site under that suffix. A curated public-suffix subset now takes three labels under known multi-label suffixes (`co.uk`, `com.au`, `github.io`, `vercel.app`, …), kept in sync across the five writer/reader sites (`background.js`, `detect-watched.js`, `popup.js`, `report.js`, `scoper/scoper.js`). The farble *seed* was already per-origin, so default protection was never affected — only exception scoping. Old over-broad keys self-heal to defaults; no migration. (No structural rule — dot/char/label count — can derive this: `github.io` and `sentry.io` are identical in shape but opposite in answer, because it's a registry decision.)
- **Legacy `per-tab` default rewritten to `rotation`.** The removed `per-tab` blur mode already folded to `rotation` at read-time; a one-time startup migration now rewrites the stored `defaultBlurMode` so the dashboard radio and direct reads reflect the current model.
...
- **README rewrite — value-first, honest framing.** Restructured away from a feature/UI tour to lead with what wearehere does *for the user* (names watchers, cleans tracker cookies, blurs device fingerprint, reads the fine print) and an honest "what we can and can't do" section — the realistic blurring ceiling, why we don't beat browser-level/anti-bot fingerprinting, the one-click relax escape hatch, and the pair-with-uBlock posture (we don't block requests). Plain language, no implementation jargon. Install steps for Chrome/Firefox kept.
- **PRDs consolidated into one** (`docs/01-product/PRD.md`). The separate Phase 2 farbler PRD (`PRD-phase2-farbler.md`) is merged in and removed. The single PRD leads with the current authoritative design (thesis/posture, architecture + consolidated storage contract, honest ceiling, the three capabilities, popup + 3-tab dashboard UX) and preserves **every** decision and finding in a chronological decision log — survey learnings, the detection→intervention reversal, all farbler Slice 2/3 findings (sandbox-iframe ceiling, receiver guard, WeakMap dedup, `navigator.platform` revert, Nike/Akamai aggregate tamper, rage-reload `onUpdated` rework, ToS-cache persistence, suffix-aware keys), POC bug-classes, and the non-committed Phase 3 candidates. Superseded decisions (`per-tab`→`rotation`, detection-only, Cookie-scoper-tab→Privacy-guard) are marked, not deleted. Known issues + deferred work consolidated into one table.
