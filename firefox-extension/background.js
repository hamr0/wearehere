...
// Firefox MV3 loads background as an event page via manifest
// `background.scripts` (no `importScripts` in scope). Chrome MV3 uses a
// service worker where `importScripts` is mandatory. Guard so the same
// file works in both — manifest dictates load order on Firefox.
