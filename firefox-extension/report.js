...
(function () {
  ...
  function loadReport(tabId) {
    ...
    browser.runtime.sendMessage(msg).then((report) => {
  ...
  function buildTabDropdown() {
    browser.runtime.sendMessage({ type: 'getOpenTabs' }).then((tabs) => {
  ...
  browser.runtime.sendMessage({ type: 'registerDashboard' });
        ...
        browser.runtime.sendMessage(
          { type: 'cleanCookies', mode: 'list', cookies: riskyCookies, url: reportData.url }
            ...
            setTimeout(function() {
              var msg = { type: 'getFullReport' };
              if (currentTabId) msg.tabId = currentTabId;
              browser.runtime.sendMessage(msg).then(function(report) {
                if (report) {
                  reportData = report;
